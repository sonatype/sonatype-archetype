    ...
    implements ContextEnabled
{
    ...
    public void execute() throws MojoExecutionException {
         ...
         * ".m2/archetype-catalog.properties" ); if ( archetypeCatalogPropertiesFile.exists( ) ) {
         * Properties archetypeCatalogProperties = PropertyUtils.loadProperties(
         * archetypeCatalogPropertiesFile ); getLog( ).debug( "Updating catalogs " +
         * archetypeCatalogProperties ); String[] sources = StringUtils.split(
         * archetypeCatalogProperties.getProperty( "sources" ), "," ); for ( int i = 0; i <
         * sources.length; i++ ) { String sourceRoleHint = sources[i].trim(); try { getLog( ).debug(
         * "Updating catalog " + sourceRoleHint ); ArchetypeDataSource source =
         * (ArchetypeDataSource) archetypeSources.get( sourceRoleHint ); source.updateCatalog(
         * getArchetypeSourceProperties( sourceRoleHint, archetypeCatalogProperties ), archetype,
         * settings ); getLog( ). info( "Updated " + sourceRoleHint + " using repository " +
         * localRepository.toString( ) ); } catch ( ArchetypeDataSourceException ex ) { getLog( ).
         * warn( "Can't update " + sourceRoleHint + " using repository " + localRepository.toString(
         * ) ); } } } else { getLog( ).debug( "Not updating wiki catalog" ); }
         */
    }
